...
class Items extends Security_Controller {
    ...
    function pricing_add_to_cart($id, $default_call = 0){

        if( empty($default_call) )
        {
            $cart_items = $this->Order_items_model->get_details(array("created_by" => $this->login_user->id))->getResult();
            foreach( $cart_items as $cart_item ) {
                $this->Order_items_model->delete($cart_item->id);
            }
        }
        // $id = $this->request->getPost("id");
         // $id = 2;
        ...
        //echo $this->login_user->id;die;

        $order_item_data = array(
            "title" => $item_info->title,
            "quantity" => get_setting('qty_min_order'),//1, //add 1 item first time
            "unit_type" => $item_info->unit_type,
            "rate" => $item_info->rate,
            "total" => $item_info->rate * get_setting('qty_min_order'), //since the quantity is 1
        ...
        if( !empty($id) && ($id != 6) && $item_info->payment_type != 'subscription' ) {
            $this->pricing_add_to_cart(6, 1);
        }

        if ($save_id) {
            app_redirect("orders/process_order");
    ...
    function count_cart_items() {
        ...
        // @Riya - In case if quantity is one then we are returning the minimum quantity
        if($cart_items_count == 1)
            $cart_items_count = get_setting('qty_min_order');
    ...
    function change_cart_item_quantity($type = "") {
        ...
        helper('cookie');
        set_cookie("default_order_note", $order_note);
        set_cookie("default_delivery_date", $delivery_date);
