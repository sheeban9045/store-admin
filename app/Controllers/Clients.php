...
class Clients extends Security_Controller
{
    ...
    /* admin can login as an client */
    function modal_clients_login()
    {
        $view_data = array();
        $view_data['client_id'] = isset($_GET['client_id']) ? $_GET['client_id'] : 0;
        return $this->template->view('clients/client_login_popup', $view_data);
    }

    function login_client_post()
    {
        $client_id = $this->request->getPost('client_id');
        if ($this->login_user->is_admin && !empty($client_id)) {
            // $session = \Config\Services::session();
            // $session->destroy();
            $user_detail = $this->Users_model->get_details(array("client_id" => $client_id));
            $session = \Config\Services::session();
            $session->set('user_id', $user_detail->getRow()->id);
            try {
    ...
    {
        $order =  $this->Orders_model->my_orders($data->id)->getRow();
        $user_email = $this->Users_model->get_one($data->id)->email;
        $user_created_at = $this->Users_model->get_one($data->id)->created_at;

        try {
            $stripeResponseObj = json_decode(str_replace("StripeCheckoutSession JSON: ", "", $order->stripe_response));
            $subscriptionId = isset($stripeResponseObj->subscription) ? $stripeResponseObj->subscription : '';

            if (empty($subscriptionId)) {
                throw new \Exception("Subscription ID is empty");
            }
            // Get Stripe keys
            ...
            if (empty($payment_setting->secret_key) || empty($payment_setting->publishable_key)) {
                throw new \Exception("Stripe API keys are missing");
            }
        ...
        if (!empty($invoices)) {
            foreach ($invoices as $invoice) {
                $totalInvoiced += $invoice['amount_due'] / 100;  // Stripe amounts are in cents
                $paymentReceived += $invoice['amount_paid'] / 100;
                $amountDue += $invoice['amount_remaining'] / 100;
            }
        }
        ...
        $primary_contact = get_client_contact_profile_link($data->primary_contact_id, $contact, array('style' => 'white-space:nowrap;text-overflow: ellipsis;width: 175px;overflow: hidden;display: block;'));
        $user_created_at_div = "<div style='white-space:nowrap;' >$user_created_at</div>";
        $group_list = "";
        ...
        $row_data = array(
            ...
            !empty($totalInvoiced) ? to_currency($totalInvoiced, $data->currency_symbol) : '--',
            // to_currency($data->invoice_value, $data->currency_symbol),
            !empty($paymentReceived) ? to_currency($paymentReceived, $data->currency_symbol) : '--',
            // to_currency($data->payment_received, $data->currency_symbol),
            !empty($amountDue) ? to_currency($amountDue, $data->currency_symbol) : '--',
        ...
        if (!empty($order)) {
            $row_data[] = modal_anchor(get_uri("clients/modal_clients_login?client_id=" . $data->id), "login", array("class" => "edit btn btn-success ", "title" => "Login As Client"));

            $row_data[] = "<a  href = 'http://" . $order->domain_name . "/index.php/login/user_email/" . $user_email . "/autologin/1' title = 'Community URL' class = 'fw-bold' target='_blank' > http://" . $order->domain_name . " </a>";


            $row_data[] = "<a href = 'http://" . $order->domain_name . "/index.php/login/user_email/" . $user_email . "/autologin/1/admin/1' title = 'Community Admin URL' class = 'btn btn-primary' target='_blank'>Admin</a>";
        } else {
            $row_data[] = "<a href = 'javascript:void(0)' title = 'No Community Available' class = 'fw-bold'> -- </a>";
            $row_data[] = "<a href = 'javascript:void(0)' title = 'No Community Available' class = 'fw-bold'> -- </a>";
            $row_data[] = "<a href = 'javascript:void(0)' title = 'No Community Available' class = 'fw-bold'> -- </a>";
        }
    ...
    function vacations($client_id)
    {
        // $this->access_only_allowed_members();
        $this->can_access_this_client($client_id);
        $get_client_holidays = $this->Client_vacations_model->get_client_holidays($client_id);

        $vacationArray = [];
        if (!empty($get_client_holidays)) {
            foreach ($get_client_holidays as $holiday) {
                $tempVacationArray['title'] = $holiday->title;
                $tempVacationArray['start'] = $holiday->start_date;
                $tempVacationArray['end'] = $holiday->end_date;

                $vacationArray[] = $tempVacationArray;
            }
        }
        ...
        if ($client_id) {
            $view_data["client_info"] = $this->Clients_model->get_one($client_id);
            $view_data['client_id'] = clean_data($client_id);
            $view_data['holidays'] = json_encode($vacationArray); // $this->Client_vacations_model->get_client_holidays($client_id); //get_setting("holidays");
            return $this->template->view("clients/vacations/index", $view_data);
        }
    }
    ...
    function type_payment($client_id)
    {
        $this->access_only_allowed_members();
        $this->can_access_this_client($client_id);


        return $this->template->view("clients/type_payment/index");
    }
    ...
    {
        //setting preferences
        $settings = array("notification_sound_volume", "disable_push_notification", "disable_keyboard_shortcuts", "personal_rtl_support");
