...
class Invoice_payments extends Security_Controller {
    ...
    function payment_modal_form() {
        ...
        $view_data['payment_methods_dropdown'] = $this->Payment_methods_model->get_dropdown_list(array("title"), "id", array("online_payable" => 1, "deleted" => 0));
    ...
    function clients_payment_summary_list_data() {
        ...
        foreach ($list_data as $data) {
            $result[] = array(
                get_client_contact_profile_link($data->client_id, $data->client_name),
                to_currency($data->amount)
            );
        }
