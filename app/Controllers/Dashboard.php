...
class Dashboard extends Security_Controller {
    ...
    public function index() {
        ...
        if ($this->login_user->user_type === "staff" && $this->show_staff_on_staff) {
            ...
            $view_data["show_project"] = get_array_value($widgets, "my_timesheet_statistics");
            ...
            if ($client_default_dashboard) {
                ...
                $view_data['client_id'] = $client_id = get_array_value($widgets, "client_id");
                ...
                $view_data['stripeResponse'] = (isset($view_data["recent_order"]) && !empty($view_data["recent_order"]))?json_decode(substr($view_data['recent_order'][0]->stripe_response,28)):'';
                $view_data["plan_detail"] = $this->Orders_model->get_plan_detail($order_id)->getRow();

                $view_data["my_orders"] =  $this->Orders_model->my_orders($client_id)->getResult();
                // echo "<pre>"; print_r($view_data["stripeResponse"]);die;

                for( $i = 0; $i < count($view_data["my_orders"]); $i++){
                    if(isset($view_data['my_orders'][$i]->stripe_response) && !empty($view_data['my_orders'][$i]->stripe_response)){

                        $view_data['my_orders'][$i]->stripe_response = json_decode(substr($view_data['my_orders'][$i]->stripe_response,28));
                    }
    ...
    private function _check_widgets_for_staffs() {
        ...
        if ($this->login_user->is_admin || $access_leads->access_type) {
