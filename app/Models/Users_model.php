...
class Users_model extends Crud_model {
    ...
    function login_user_id() {
        $session = \Config\Services::session();
        $_SESSION["uid"] = $session->has("user_id") ? $session->get("user_id") : "";
    ...
    function get_user_from_full_name($user_full_name = "", $user_type = "") {
        ...
        $sql = "SELECT 'name'
        FROM 'edomain'
        WHERE $edomain.deleted=0 AND $edomain.status='active' AND CONCAT(TRIM($edomain.name), ' ', TRIM($edomain.name))='$user_full_name' $where
        LIMIT 1";
