...
class Left_menu {
    ...
    private function _prepare_sidebar_menu_items($type = "", $return_sub_menu_data = false) {
        ...
        // @Riya_17062022: following menu added to manage the holiday or vacations
        // $final_items_array["holiday"] = array("name" => "holiday", "url" => "holiday_vacation", "class" => "check-square");
    ...
    private function _get_sidebar_menu_items($type = "") {
        ...
        if ($this->ci->login_user->user_type == "staff" && $type !== "client_default") {
            ...
            if (get_setting("module_invoice") == "1" && ($this->ci->login_user->is_admin || $access_invoice)) {
              ...
              if (get_setting("module_domain") == "1" && ($this->ci->login_user->is_admin || $access_domain)) {
                $sales_submenu[] = array("name" => "create_domain", "url" => "create_domain", "class" => "dollar-sign");
                $show_payments_menu = true;
            }
            ...
            if ($show_expenses_menu && $show_payments_menu) {
                $reports_submenu[] = array("name" => "income_vs_expenses", "url" => "expenses/income_vs_expenses", "class" => "trendng-up");
            }
